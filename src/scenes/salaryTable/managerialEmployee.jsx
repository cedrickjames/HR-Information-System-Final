...
function EnhancedTableHead(props) {
  const { class1, class2, increment, daily1, onRequestSort } =
    props;
...
const { numSelected,  dailyOne, dailyTwo, dailyThree, increment1, increment2, increment3} = props;
...
<div className="w-fit" data-testid="flowbite-tooltip-target"><button type="button" onClick={()=> showOption()}
    ...
    <Modal
      ...
      >
       ...
       <TextField    label="M3 Increment" value = {d1value} onChange={(e) => setD1Value(e.target.value)}    fullWidth />
       <TextField    label="M4 Increment" value = {d2value} onChange={(e) => setD2Value(e.target.value)}  fullWidth />
       <TextField    label="M5 Increment" value = {d3value} onChange={(e) => setD3Value(e.target.value)}  fullWidth />
    ...
    <Modal
      ...
      >
       ...
       <TextField    label="M5 level 1" value = {dailyValue3} onChange={(e) => setDailyValue3(e.target.value)}  fullWidth />
...
console.log (props);
  ...
  const { m1, m2, m3, m4, m5, m1l1, m2l1, m3l1, m4l1 , m5l1, workingDays } = props;
  ...
  const [d1value, setD1Value] = React.useState(m3);
  const [d2value, setD2Value] = React.useState(m4);
  const [d3value, setD3Value] = React.useState(m5);
  ...
  var daily = (i-1)*parseFloat(m1)+parseFloat(m1l1)
  var monthly = Math.round(parseFloat(workingDays) * parseFloat (daily));
  var daily2 = (i-1)*parseFloat(m2)+parseFloat(m2l1)
  var monthly2 = Math.round(parseFloat(workingDays) * parseFloat (daily2));
  var daily3 = (i-1)*parseFloat(m3)+parseFloat(m3l1)
  var monthly3 = Math.round(parseFloat(workingDays) * parseFloat (daily3));
  
  var daily4 = (i-1)*parseFloat(m4)+parseFloat(m4l1)
  var monthly4 = Math.round(parseFloat(workingDays) * parseFloat (daily4));
  console.log(daily4, monthly4);
  var daily5 = (i-1)*parseFloat(m5)+parseFloat(m5l1)
  var monthly5 = Math.round(parseFloat(workingDays) * parseFloat (daily5));
  ...
  return (
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        <EnhancedTableToolbar numSelected={selected.length}  dailyOne = {m3l1} dailyTwo = {m4l1} dailyThree = {m5l1} increment1 = {m3} increment2 = {m4}  increment3 = {m5} />
          ...
          >
             ...
             */}
              {visibleRows.map((row, index) => {
                ...
                const rowColor = isEven ? '#cfcfcf' : '#fffe9d';
          ...
          >
             ...
             */}
              {visibleRows.map((row, index) => {
                ...
                const labelId = `enhanced-table-checkbox-${index}`;
                const isEven = index % 2 === 0;
                const rowColor = isEven ? '#a3a3a3' : '#fff666';
          ...
          >
             ...
             */}
              {visibleRows.map((row, index) => {
                ...
                const labelId = `enhanced-table-checkbox-${index}`;
                const isEven = index % 2 === 0;
                const rowColor = isEven ? '#919191' : '#f1da22';
