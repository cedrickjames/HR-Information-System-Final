...
function createData(id, name, status, username) {
  return {
    id,
   name,
   status,
...
function EnhancedTableHead(props) {
  ...
  return (
    <TableHead>
      <TableRow >
        <TableCell style={{  border: '1px solid black', fontWeight: 'bold', fontSize: '16px' }} rowSpan={2}  align='left' padding='normal'>
...
const UsersTable = (props ) => {
  ...
  function updateUserStatus (status, id){
    Axios.post("http://192.168.60.53:3001/updateUserStatus", {
      userStatus: status,
      id: id,
  ...
  const refreshTable1 = () => {
    ...
    Axios.post("http://192.168.60.53:3001/users", {
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <TableCell style={{  border: '1px solid black', fontWeight: 'bold', fontSize: '16px' }}   align="left">{row.status === 1 ? 'Activated' : 'Deactivated'}</TableCell>
                    <TableCell style={{  border: '1px solid black', fontWeight: 'bold', fontSize: '16px' }}   align="left">{row.username}</TableCell>

                    <TableCell style={{ maxWidth:'50px', border: '1px solid black', fontWeight: 'bold', fontSize: '16px' }}   align="left">
                    {row.status === 1 ?  <button
     onClick={()=> updateUserStatus(0, row.id)}
     type="button"
     className=" text-white h-10 w-56 bg-gradient-to-br from-red-600 to-red-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg px-5 py-2.5 text-center mr-2 "
   >
    Disapproved
   </button> :  <button
      onClick={()=> updateUserStatus(1,row.id)}
     type="button"
     className=" text-white h-10 w-56 bg-gradient-to-br from-purple-600 to-blue-500 hover:bg-gradient-to-bl focus:ring-4 focus:outline-none focus:ring-blue-300 dark:focus:ring-blue-800 font-medium rounded-lg px-5 py-2.5 text-center mr-2 "
   >
    Approved
   </button>}
